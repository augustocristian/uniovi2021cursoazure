...
Para crear una aplicación web con Flutter tenemos que utilizar su canal "Beta", ya que por el momento, esta no se encuentra soportada en la rama estable oficial. Para habilitar este "canal" tenemos que realizar los siguientes comandos
 ```
flutter channel beta
flutter upgrade
flutter config --enable-web
 ```
Una vez hecho esto, ya podríamos compilar el proyecto,  para ello primero creamos todos los directorios necesarios con:
 ```
 flutter create .
 ```
 Para acto seguido construir la solución con:
 ```
 $ flutter build web
 ```
...
1. Creamos una nueva rama de desarrollo y nos cambiamos a ella:
 ```
...
7. Ahora vamos a desplegar el servidor NodeJS que se ha creado en AppServices.Azure permite integraciones con una gran cantidad de fuentes, por lo que automatizar el despliegue de una aplicación en un repositorio no debería de ser un problema. Nos dirigimos Azure App Service y seleccionamos usar GitHub con Azure Portal (esto permitirá acceder directamente al código del repositorio).
...
* Versión Node: 14 LTS
Pulsamos Finalizar
8. Nos dirigimos a la URL donde tenemos nuestro recurso de Azure y ¡Voila!
<p align="center">
  <img src="./leemeimages/11_ResourceDeployedOnTheCloud.PNG"  >
</p>
9. Ahora vamos a comprobar el "Despliegue continuo", para ello vamos a modificar algún detalle y subirlo al repositorio, por ejemplo el fondo de la aplicación (ubicado en el directorio /images con el nombre "background.png"). 

10. Debemos de volver a compilar la página web, para ello ejecutamos una vez más:
 
  ```
  flutter build web
   ```

11. Hacemos push y comprobamos que se actualice (puede tardar varios minutos en ocasiones.
<p align="center">
